resource "aws_elastic_beanstalk_application" "e9_ba_1" {
  name        = "e9-ba-1"
  description = "e9 beanstalk application v1"
}

resource "aws_elastic_beanstalk_environment" "e9_be_1" {
  name                = "e9-be-1"
  ...
  setting {
    namespace = "aws:autoscaling:launchconfiguration"
    name      = "SecurityGroups"
    value     = aws_security_group.e9-bean-sg.id
  }
  ...
  depends_on = [aws_security_group.elb-sg, aws_security_group.e9-bean-sg]
